# Title:Calculation of the upper limit of the mesophotic zone by site of the Mexican Pacific
...
setwd("") # set work directory where the files are included 
...
##### CALCULATIONS OF THE UPPER LIMIT ESTIMATE OF THE MESOPHOTIC ZONE BY LOCATION #####
# ___________________________________________________________________________________ #
...
data <- data %>%
  left_join(I0, by = c("site", "station")) %>%
  mutate(
...
# This instruction summarizes the KdPAR calculation and returns the maximum (maz),
...
Kd_PAR <- Kd490 %>%
  ...
  summarise(
    ...
    KdPAR_mean = mean(KdPAR, na.rm = TRUE),    # average value
...
z10_KdPAR <- Kd490 %>%
  ...
  summarise(
    ...
    prom_z10 = mean(z10, na.rm = TRUE),   # average value
...
satellite_Irel <- Kd_PAR %>%
  left_join(Irel_max_depth %>% select(site, station, max_depth_obs), by = c("site", "station")) %>%
  mutate(
  ...
  select(site, station, Irel_sat_mean, Irel_sat_min, Irel_sat_max)

# Results
# This instruction summarizes the results into a single dataframe and saves it to an Excel sheet.
results_kd <- Kd_PAR %>%                                      # Data frame Kd_PAR
  left_join(I0, by = c("site","station")) %>%                 # Data frame I0
  left_join(z10_KdPAR, by = c("site", "station")) %>%         # Data frame z10_KdPAR
  left_join(z10_tag, by = c("site","station")) %>%            # Data frame z10_tag
  left_join(Irel_max_depth, by = c("site", "station")) %>%    # Data frame Irel_max_depth
  left_join(satellite_Irel, by = c("site", "station")) %>%    # Data frame satellite_Irel
  select(
    Site = site,
    Station = station,
    ...
    Average_KdPAR_from_Kd490 = KdPAR_mean,
    Minimum_KdPAR_from_Kd490 = KdPAR_min,
    Maximum_Z10_from_KdPAR = max_z10,
    Average_Z10_from_KdPAR = prom_z10,
...
write.xlsx(results_kd, "Results_upper_limit_of_the_mesophotic_zone_of_the_Mexican_Pacific_by_site.xlsx", overwrite = TRUE)
