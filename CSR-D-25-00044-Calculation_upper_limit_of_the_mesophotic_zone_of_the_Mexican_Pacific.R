# Title:  Calculation of the upper limit of the mesophotic zone of the Mexican Pacific
...
satellite_Irel <- Kd_PAR %>%
  left_join(Irel_max_depth %>% select(region, max_depth_obs), by = "region") %>%
  mutate(
  ...
  select(region, Irel_sat_mean, Irel_sat_min, Irel_sat_max)

# Results
# This instruction summarizes the results into a single dataframe and saves it to an Excel sheet.
results_kd <- Irel_max_depth %>%                    # Data frame Irel_max_depth
  left_join(I0, by = c("region" )) %>%              # Data frame I0
  left_join(z10_KdPAR, by = "region") %>%           # Data frame z10_KdPAR
  left_join(z10_tag, by = "region") %>%             # Data frame z10_tag
  left_join(Kd_PAR, by = "region") %>%              # Data frame Kd_PAR
  left_join(satellite_Irel, by = "region") %>%      # Data frame satellite_Irel
  select(
    Region = region,
...
write.xlsx(results_kd, "Results_upper_limit_of_the_mesophotic_zone_of_the_Mexican_Pacific.xlsx", overwrite = TRUE)
